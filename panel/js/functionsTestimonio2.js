$(function() {
	...
	$('.buttonagregar').click(function(event) {
		mdlLista.find('.modal-title').html('Nuevo Slide');
		mdlLista.find('#id').val('');
		mdlLista.find('#operaciones').val('agregartestimonio2');
	...
	$('#table-short').on('click', '.edit', function(event) {
		...
		mdlLista.find('.modal-title').html('Editar Slide');
		mdlLista.find('#id').val(_data.id);
		mdlLista.find('#operaciones').val('actualizartestimonio2');
		...
		mdlLista.find('#preview-testimonio').html('<img src="../img/imgTestimonio2/'+ _data.portada +'" class="img-responsive">');
