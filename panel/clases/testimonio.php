...
class testimonio extends Archivo {
	...
	var $directorio = "../img/imgTestimonio/";
	...
	function agregarTestimonio() {
		...
		if ($this -> subir_archivo_imagen($this -> imgPortada)) {
			...
			$con = new conexion();
			$sql = "INSERT testimonio SET imgPortada = '{$this -> imgPortada}', nombre = '{$this -> nombre}', ubicacion = '{$this -> ubicacion}', texto = '{$this -> texto}', idioma = '{$this -> idioma}', textoEn = '{$this -> textoEn}', idDestino = '{$this -> idDestino}'";
			
			$this -> idTestimonio = $con -> ejecutar_sentencia($sql);

			if ($this -> idTestimonio > 0) {
				$sql2 = "UPDATE testimonio SET orden = {$this -> idTestimonio} WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function editarTestimonio() {
		...
		if ($this -> imgPortada !== '') {
			...
			$this -> ruta_final = $this -> directorio;
			if ($this -> subir_archivo_imagen($this -> imgPortada)) {
				$img = "UPDATE testimonio SET imgPortada = '{$this -> imgPortada}' WHERE idTestimonio = {$this -> idTestimonio}";
	...
	// 		$_MYSQL = new MYSQL();
	// 		$_SQL = "UPDATE testimonio SET imgMovil = ? WHERE idTestimonio = ?";
	...
	function borrarTestimonio() {
		...
		$success = 0; 
		$con     = new conexion();
		$sql     = "DELETE FROM testimonio WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function obtenerPortada() {
		$sql  = "SELECT imgPortada FROM testimonio WHERE idTestimonio = {$this -> idTestimonio}";
		$con  = new conexion();
	...
	// 	$_MYSQL = new MYSQL();
	// 	$_SQL = "SELECT imgMovil FROM testimonio WHERE idTestimonio = ?";
	// 	$_CONECTADO = $_MYSQL -> Connect();
	// 	if (!$_CONECTADO) {
	// 		echo 'Ocurrio un error, Por favor intentalo mas tarde.';
	// 		exit();
	...
	function cambiarStatusTestimonio($status = 0) {
		$con = new conexion();
		$sql = "UPDATE testimonio SET status = {$status} WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function editarOrdenTestimonio($orden = 0) {
		$con = new conexion();
		$sql = "UPDATE testimonio SET orden = {$orden} WHERE idTestimonio = {$this -> idTestimonio}";
		$con -> ejecutar_sentencia($sql);
	}
	...
	function obtenerTestimonio() {
		$con      = new conexion();
		$sql      = "SELECT * FROM testimonio WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function listaTestimonio($pagina = 1, $paginador = true, $status = '', $idDestino = 0, $idioma = 'en', $busqueda = '', $registrosPorPagina = 20) {
		...
		$sql = "SELECT * FROM testimonio WHERE idDestino='".$idDestino."' ".$stat.$bus." ORDER BY orden ASC";
		$con = new conexion();
		$temporal = $con -> ejecutar_sentencia($sql);
	...
	public function listaTestimonioAleatorios($idioma = 'en', $registros = 5) {
		$con    = new conexion();
		$sql1   = "SELECT `idTestimonio` FROM `testimonio` WHERE `status` = 1 AND `idioma` = '{$idioma}'";
		...
		$sql2   = "SELECT * FROM testimonio WHERE status = 1 AND idioma LIKE '{$idioma}' AND idTestimonio IN({$in}) LIMIT {$registros}";
