...
class slide extends Archivo{
	...
	function addSlide(){
		if($this -> subir_archivo_imagen($this -> _imgPortada)){
			$_MYSQL = new MYSQL();
			$_SQL = "INSERT INTO slide(titulo, link, imgPortada, status, tituloEn, descripcion, descripcionEn, textoBoton, textoBotonEn, linkVideo, tipo)VALUES(?,?,?,?,?,?,?,?,?,?,?)";
			...
			if($_MYSQL -> Execute($_SQL, array($this -> _titulo, $this -> _link, $this -> _imgPortada, 1, $this -> _tituloEn, $this -> _descripcion, $this -> _descripcionEn, $this -> _textoBoton, $this -> _textoBotonEn, $this -> _linkVideo, $this -> _tipo))){
				...
				$_O = "UPDATE slide SET orden = ? WHERE idSlide = ?";
	...
	function updateSlide(){
		...
		if($this -> _imgPortada != ''){
			...
			$this -> ruta_final = $this -> _directorio;
			if($this -> subir_archivo_imagen($this -> _imgPortada)){
				$_IMG = "UPDATE slide SET imgPortada = ? WHERE idSlide = ?";
				$_CONECTADO = $_MYSQL -> Connect();
				if(!$_CONECTADO){
					echo 'Ocurrio un error, Por favor intentalo mas tarde.';
					exit();
		...
		$_SQL = "UPDATE slide SET titulo = ?, link = ?, tituloEn = ?,  descripcion = ?,  descripcionEn = ?,  textoBoton = ?,  textoBotonEn = ?, linkVideo = ? WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function updateImgMovil($_ruta = '', $_tmp = ''){
		if($_ruta != ''){
			...
			$_MYSQL = new MYSQL();
			$_SQL = "UPDATE slide SET imgMovil = ? WHERE idSlide = ?";
			$_CONECTADO = $_MYSQL -> Connect();
			if(!$_CONECTADO){
				echo 'Ocurrio un error, Por favor intentalo mas tarde.';
				exit();
	...
	function deleteSlide(){
		...
		$_MYSQL = new MYSQL();
		$_SQL = "DELETE FROM slide WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function getImgPortada(){
		$_MYSQL = new MYSQL();
		$_SQL = "SELECT imgPortada FROM slide WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function getImgMovil(){
		$_MYSQL = new MYSQL();
		$_SQL = "SELECT imgMovil FROM slide WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function updateStatusSlide($_status){
		$_MYSQL = new MYSQL();
		$_SQL = "UPDATE slide SET status = ? WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function updateOrdenSlide($_orden){
		$_MYSQL = new MYSQL();
		$_SQL = "UPDATE slide SET orden = ? WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function getSlide(){
		$_MYSQL = new MYSQL();
		$_SQL = "SELECT * FROM slide WHERE idSlide = ?";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
	...
	function listSlide($_pagina = 1, $_paginador = true, $_status = '', $_busqueda = '', $_registrosPorPagina = 20, $_lastID = '', $tipo=1){
		...
		$_MYSQL = new MYSQL();
		$_TOTAL = "SELECT idSlide, count(idSlide) as totalRegistros FROM slide WHERE 1 = 1 ".$_stat." ".$_bus." ".$_tip." ORDER BY orden DESC";
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
		...
		$_SQL = "SELECT * FROM slide WHERE 1 = 1 ".$_stat.$_bus.$_tip." ORDER BY orden DESC ".$_paginacion;
		$_CONECTADO = $_MYSQL -> Connect();
		if(!$_CONECTADO){
			echo 'Ocurrio un error, Por favor intentalo mas tarde.';
			exit();
