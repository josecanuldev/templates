...
class testimonio3 extends Archivo {
	...
	var $directorio = "../img/imgTestimonio3/";
	...
	function agregarTestimonio() {
		...
		if ($this -> subir_archivo_imagen($this -> imgPortada)) {
			...
			$con = new conexion();
			$sql = "INSERT testimonio3 SET imgPortada = '{$this -> imgPortada}', nombre = '{$this -> nombre}', ubicacion = '{$this -> ubicacion}', texto = '{$this -> texto}', idioma = '{$this -> idioma}', textoEn = '{$this -> textoEn}', idDestino = '{$this -> idDestino}'";

			$this -> idTestimonio = $con -> ejecutar_sentencia($sql);

			if ($this -> idTestimonio > 0) {
				$sql2 = "UPDATE testimonio3 SET orden = {$this -> idTestimonio} WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function editarTestimonio() {
		...
		if ($this -> imgPortada !== '') {
			...
			$this -> ruta_final = $this -> directorio;
			if ($this -> subir_archivo_imagen($this -> imgPortada)) {
				$img = "UPDATE testimonio3 SET imgPortada = '{$this -> imgPortada}' WHERE idTestimonio = {$this -> idTestimonio}";
	...
	// 		$_MYSQL = new MYSQL();
	// 		$_SQL = "UPDATE testimonio3 SET imgMovil = ? WHERE idTestimonio = ?";
	...
	function borrarTestimonio() {
		...
		$success = 0;
		$con     = new conexion();
		$sql     = "DELETE FROM testimonio3 WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function obtenerPortada() {
		$sql  = "SELECT imgPortada FROM testimonio3 WHERE idTestimonio = {$this -> idTestimonio}";
		$con  = new conexion();
	...
	// 	$_MYSQL = new MYSQL();
	// 	$_SQL = "SELECT imgMovil FROM testimonio3 WHERE idTestimonio = ?";
	// 	$_CONECTADO = $_MYSQL -> Connect();
	// 	if (!$_CONECTADO) {
	// 		echo 'Ocurrio un error, Por favor intentalo mas tarde.';
	// 		exit();
	...
	function cambiarStatusTestimonio($status = 0) {
		$con = new conexion();
		$sql = "UPDATE testimonio3 SET status = {$status} WHERE idTestimonio = {$this -> idTestimonio}";
	...
	function editarOrdenTestimonio($orden = 0) {
		$con = new conexion();
		$sql = "UPDATE testimonio3 SET orden = {$orden} WHERE idTestimonio = {$this -> idTestimonio}";
		$con -> ejecutar_sentencia($sql);
	}
	...
	function obtenerTestimonio() {
		$con      = new conexion();
		$sql      = "SELECT * FROM testimonio3 WHERE idTestimonio = {$this -> idTestimonio}";
	 ...
	 * @return  [type]                        [description]
	 */
	function listaTestimonio($pagina = 1, $paginador = true, $status = '', $idDestino = 0, $idioma = 'en', $busqueda = '', $registrosPorPagina = 20,$orden='') {
		$stat   = ($status != '') ? "AND status = ".$status : '';
		$bus    = ($busqueda != '') ? "AND (nombre LIKE '%".$busqueda."%')" : '';

		if($orden != ''){
			if($orden==1){
				$parteOrden="ORDER BY orden DESC";
			}
			else if($orden==2){
				$parteOrden="ORDER BY orden ASC";
			}
		}else{
			$parteOrden="ORDER BY orden DESC";
		}

		$sql = "SELECT * FROM testimonio3 WHERE 1=1 ".$stat.$bus." ".$parteOrden." ";
		$con = new conexion();
		$temporal = $con -> ejecutar_sentencia($sql);
	...
	public function listaTestimonioAleatorios($idioma = 'en', $registros = 5) {
		$con    = new conexion();
		$sql1   = "SELECT `idTestimonio` FROM `testimonio3` WHERE `status` = 1 AND `idioma` = '{$idioma}'";
		...
		$sql2   = "SELECT * FROM testimonio3 WHERE status = 1 AND idioma LIKE '{$idioma}' AND idTestimonio IN({$in}) LIMIT {$registros}";
