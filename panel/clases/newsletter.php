...
class newsletter{
	...
	function __construct($idNewsletter = 0, $correo = 0, $tipo = ''){
	...
	function addNewsletter(){
		if($this -> validNewsletter($this -> correo, $this -> tipo)){
			...
			$_MYSQL = new MYSQL();
			$_SQL = "INSERT INTO newsletter(correo, tipo, fechaCreacion, status) VALUES (?,?,?,?)";
			$_VALUES = array($this -> correo, $this -> tipo, $_currentDate, 1);
	...
	function listNewsletter($_pagina = 1, $_paginador = true, $_status = '', $_busqueda = '', $_registrosPorPagina = 20, $_frontEnd = false, $_tipo = ''){
		...
		if($_paginador){
			$_TOTAL = "SELECT idNewsletter, count(idNewsletter) as totalRegistros FROM newsletter WHERE 1 = 1 ".$_t.$_stat." ".$_bus." ORDER BY orden DESC";
			$_CONECTADO = $_MYSQL -> Connect();
			if(!$_CONECTADO){
				echo 'Ocurrio un error, Por favor intentalo mas tarde.';
				exit();
		...
		$_SQL = "SELECT * FROM newsletter WHERE 1 = 1 ".$_t.$_stat.$_bus." ORDER BY orden DESC ".$_paginacion;
	...
	function validNewsletter($_correo = '', $_tipo = ''){
		$_MYSQL = new MYSQL();
		$_SQL = "SELECT idNewsletter FROM newsletter WHERE correo = ? AND tipo = ?";
