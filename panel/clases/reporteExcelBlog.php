<?php
    ...
    require_once 'newsletterblog.php';
    ...
    if (PHP_SAPI == 'cli')
        ...
        $newsletter = new newsletterblog();
        $temporal = $newsletter -> listNewsletter(1,'',true);

        // Se asignan las propiedades del libro
        $objPHPExcel->getProperties()->setCreator("Holbox") //Autor
                             ->setLastModifiedBy("Holbox") //Ultimo usuario que lo modificó
                             ->setTitle("Newsletter")
                             ->setSubject("Holbox")
                             ->setDescription("Newsletter")
                             ->setKeywords("Newsletter")
                             ->setCategory("Reporte excel blog");

        $tituloReporte = "Newsletter Blog Holbox";
        $titulosColumnas = array('ID', 'correo', 'nombre', 'pais', 'ciudad', 'genero', 'tema');
        ...
        foreach ($temporal as $key) {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C'.$i,  $key['nombre'])
                    ->setCellValue('D'.$i,  $key['pais'])
                    ->setCellValue('E'.$i,  $key['ciudad'])
                    ->setCellValue('F'.$i,  $key['genero'])
                    ->setCellValue('G'.$i,  $key['tema']);
                    $i++;
        }
        ...
        $objPHPExcel->getActiveSheet()->setTitle('Grupo Logra');
