...
class ToRtf
{
	...
	{//-- CARGAMOS VALORES POR DEFECTO
		$this->fichero ='CONVENIO-OUTLET-PLANTILLA-ORIGINAL.rtf';
	...
	{//-- CARGAMOS EL FICHERO EN UNA VARIABLE
		if(is_file($this->fichero)){
			...
			echo $this->error = 'Archivo de Origen no existe';
			return false;
		}
	}
	function rtf(){		
		...
		$txtplantilla = $this->leerArchivo();
		//echo $txtplantilla;
		if($txtplantilla = $this->leerArchivo()){//-- COMPROBAMOS SI SE CARGO BIEN EL FICHERO
			$punt = fopen($this->dirsalida.$this->fsalida,"a");//-- CREAMOS EL NUEVO FICHERO
