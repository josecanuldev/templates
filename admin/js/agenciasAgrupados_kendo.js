...
$(document).ready(function() {
	$("#grid").kendoGrid({
		dataSource: {
			...
			transport: {
				read: {
					url: baseUrl + "/agencies/getListAgencies",
					type: 'GET',
					data: function(){
						return {
							hoy: $("#hoy").val(),
			...
			sort: { field: "agencia", dir: "asc"},
			batch: true,
			group: { field: "agencia", aggregates: [
		...
		columns: [{
			...
			hidden: true,
			groupHeaderTemplate: "<span class='group--header'> #= value # - Costo: #= kendo.format('{0:c}', aggregates.total.sum) # MXN </span>",
			// media: mediaqueries 
		}, {
			field: "fecha",
			title: "Fecha",
			...
			attributes: attributes,
			template: function(dataItem) {
				var html = '<span style="font-size: 12px;" class="badge '
				switch(dataItem.estatus) {
					case 'PA':
						html += 'badge badge-success">Pagado'
						break
					case 'C':
						html += 'badge badge-danger">Cancelado'
						break
					case 'P':
					case 'A':
						html += 'badge badge-warning">Pendiente'
						break
				}
				html += '<span>'
